...
let rec infer_expression state subs = function
  ...
  | Ast.Annotated (e, ty) ->
      let _ty, subs' = check_expression state subs ty e in
      (ty, subs')
  | Ast.Tuple exprs ->
  ...
  | Ast.Variant (lbl, e) -> (
      ...
      | Some ty_in', Some expr ->
          let _ty_e, subs' = check_expression state subs ty_in' expr in
          let ty_out' = apply_subs subs' ty_out in
          (ty_out', subs')
      | None, Some _ | Some _, None ->
          Error.typing "Variant optional argument mismatch" )

and check_expression state subs annotation expr =
  match (expr, annotation) with
  | Ast.Tuple exprs, Ast.TyTuple annos ->
      let fold' (tys', subs') anno expr =
        let ty', sub' = check_expression state subs' anno expr in
        (ty' :: tys', sub')
      in
      let tys, subs' = List.fold_left2 fold' ([], subs) annos exprs in
      (Ast.TyTuple tys, subs')
  | Ast.Lambda abs, Ast.TyArrow (ty_in, ty_out) ->
      let ty_abs, subs_abs =
        check_check_abstraction state subs (ty_in, ty_out) abs
      in
  ...
  | Ast.Variant (lbl, e), _ -> (
      ...
      | None, None ->
          ...
          (annotation, subs')
      | Some ty_in', Some expr ->
          let _ty_e, subs_e = check_expression state subs ty_in' expr in
  ...
  | ((Ast.Var _ | Ast.Const _ | Ast.Annotated _) as e), _ ->
      ...
      let subs_a = unify state subs_e ty annotation in
      (ty, subs_a)
  | _, Ast.TyParam p ->
      let ty, subs' = infer_expression state subs expr in
      (ty, extend_subs (p, ty) subs')
  | _, _ ->
      let print_param = Ast.new_print_param () in
...
and infer_computation state subst = function
  ...
  | Ast.Apply (e1, e2) -> (
      ...
      match ty_1 with
      | Ast.TyArrow (ty_in, ty_out) ->
          let _ty_2, subs_2 = check_expression state subs_1 ty_in e2 in
  ...
  | Ast.Out (op, e, comp) | Ast.In (op, e, comp) ->
      ...
      let _ty, subs_e = check_expression state subst ty_op e in
  ...
  | Ast.Await (e, abs) -> (
      ...
      | Ast.TyPromise ty1 ->
          let ty_c, subs_c = check_infer_abstraction state subs_e ty1 abs in
          (ty_c, subs_c)
  ...
  | Ast.Match (e, case :: cases) ->
      ...
      let ty_1, subs_1 = check_infer_abstraction state subs_e ty_e case in
      let fold' subs' case' =
        let ty_current, subs_current =
          check_infer_abstraction state subs' ty_e case'
        in
  ...
  | Ast.Promise (op, abs, p, comp) ->
      let ty_op = Ast.OperationMap.find op state.operations in
      let ty_a, subs_a = check_infer_abstraction state subst ty_op abs in
...
and check_computation state subs annotation = function
  ...
  | Ast.Do (comp1, comp2) ->
      ...
      let ty_2, subs_2 =
        check_check_abstraction state subs_1 (ty_1, annotation) comp2
      in
      (ty_2, subs_2)
  | Ast.Apply (e1, e2) -> (
      ...
      | Ast.TyArrow (ty_in, ty_out) ->
          let _ty_2, subs_2 = check_expression state subs_1 ty_in e2 in
          let subs_anno = unify state subs_2 ty_out annotation in
          (ty_out, subs_anno)
      | _ -> Error.typing "First expresion in apply need to be of type arrow." )
  | Ast.Out (op, e, comp) | Ast.In (op, e, comp) ->
      let ty_o = Ast.OperationMap.find op state.operations in
      let _ty_e, subs_e = check_expression state subs ty_o e in
      let ty_comp, subs_comp = check_computation state subs_e annotation comp in
      (ty_comp, subs_comp)
  | Ast.Await (e, abs) -> (
      ...
      | Ast.TyPromise ty ->
          let ty_2, subs_2 =
            check_check_abstraction state subs_1 (ty, annotation) abs
          in
          (ty_2, subs_2)
  ...
  | Ast.Match (e, case :: cases) ->
      ...
      let ty_1, subs_1 =
        check_check_abstraction state subs_e (ty_e, annotation) case
      in
      let fold' subs' case' =
        let ty_current, subs_current =
          check_check_abstraction state subs' (ty_e, annotation) case'
        in
        let subs_match = unify state subs_current ty_1 ty_current in
        subs_match
      in
      let subs' = List.fold_left fold' subs_1 cases in
      (ty_1, subs')
  | Ast.Promise (op, abs, p, comp) ->
      ...
      let ty_2, subs_2 = check_infer_abstraction state subs ty_1 abs in
      let state' = extend_variables state [ (p, ty_2) ] in
      let ty_3, subs_3 = check_computation state' subs_2 annotation comp in
      (ty_3, subs_3)

and check_infer_abstraction state subs ty_argument (pat, comp) =
  let vars = check_pattern state subs ty_argument pat in
  let state' = extend_variables state vars in
  let ty_c, subs_c = infer_computation state' subs comp in
  (ty_c, subs_c)

and check_check_abstraction state subs (ty_argument, ty_comp) (pat, comp) =
  let vars = check_pattern state subs ty_argument pat in
  let state' = extend_variables state vars in
  let ty_c, subs_c = check_computation state' subs ty_comp comp in
  (ty_c, subs_c)
