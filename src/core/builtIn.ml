...
let get_reference = function
  | Ast.Reference r -> r
  | expr ->
      Error.runtime "Reference expected but got %t" (Ast.print_expression expr)
...
let functions =
  [
    ...
    int_int_to_int "(/)" ( / );
    ( "ref",
      poly_type (fun a -> Ast.TyArrow (a, Ast.TyReference a)),
      fun v -> Ast.Return (Ast.Reference (ref v)) );
    ( "(!)",
      poly_type (fun a -> Ast.TyArrow (Ast.TyReference a, a)),
      fun v ->
        let r = get_reference v in
        Ast.Return !r );
    ( "(:=)",
      poly_type (fun a ->
          Ast.TyArrow (Ast.TyTuple [ Ast.TyReference a; a ], Ast.TyTuple [])),
